...
export default async function WorkspacePage({ params }: { params: Promise<{ workspaceSlug: string }> }) {
  ...
  const initialCompanies: Company[] = records.map((record, index) => {
    ...
    const companyMethods = methods.filter((method) => method.ownerId === record.id);
    const personMethods = person ? peopleMethods.filter((method) => method.ownerId === person.id) : [];
    const values = (source: typeof companyMethods, kind: "phone" | "email") => source.filter((method) => method.kind === kind).sort((left, right) => Number(right.isPrimary) - Number(left.isPrimary)).map((method) => method.displayValue);
    const companyPhones = values(companyMethods, "phone");
    const companyEmails = values(companyMethods, "email");
    const contactPhones = values(personMethods, "phone");
    const contactEmails = values(personMethods, "email");
    const website = companyMethods.find((method) => method.kind === "website" && method.isPrimary) ?? companyMethods.find((method) => method.kind === "website");
    const socialMedia = companyMethods.filter((method) => ["linkedin", "x", "instagram", "facebook", "whatsapp", "other"].includes(method.kind)).map((method) => method.displayValue);
    return { id: record.id, name: record.companyName, initials: record.companyName.split(" ").slice(0, 2).map((part) => part[0]).join("").toUpperCase(), contact: person?.fullName ?? "", role: person?.jobTitle ?? "", department: person?.department ?? "", phone: contactPhones[0] ?? companyPhones[0] ?? "", email: contactEmails[0] ?? companyEmails[0] ?? "", companyPhones, companyEmails, contactPhones, contactEmails, site: website?.displayValue ?? record.websiteUrl ?? "", location: locations.find((location) => location.ownerId === record.id)?.line1 ?? record.physicalAddressSummary ?? "", category: record.category ?? record.industry ?? "Uncategorized", industry: record.industry ?? "", tagline: record.tagline ?? "", description: record.description ?? "", socialMedia, otherInformation: record.otherInformation, services: services.filter((service) => service.companyId === record.id).map((service) => service.name), accent: colors[index % colors.length], added: record.createdAt.toLocaleDateString(), originalImages: cardImages.filter((image) => image.companyId === record.id).map((image) => ({ id: image.id, side: image.side, url: `/api/card-images/${image.id}?workspace=${encodeURIComponent(workspaceSlug)}` })) };
  });
