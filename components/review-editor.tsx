...
export function ReviewEditor({ workspaceSlug, cardId, extraction, imageUrls }: ReviewEditorProps) {
  ...
  const reviewCount = [extraction.company.name, extraction.company.industry, extraction.company.category, extraction.company.website, extraction.company.tagline, extraction.company.productsServices, extraction.contact.fullName, extraction.contact.jobTitle, extraction.contact.phoneNumbers, extraction.contact.emailAddresses, extraction.company.physicalAddress, extraction.otherInformation].filter((field) => field.needsReview || field.confidence < .85).length;
  ...
  return <div className="review-page">
    ...
    <div className="review-layout">
      ...
      <main className="review-form">
        <section className="review-section"><div className="review-section-title"><span className="section-number">01</span><div><h2>Company</h2><p>Business identity and details</p></div></div><div className="field-grid"><ReviewField label="Company name" value={form.companyName} onChange={set("companyName")} confidence={extraction.company.name.confidence} /><ReviewField label="Industry" value={form.industry} onChange={set("industry")} confidence={extraction.company.industry.confidence} /><ReviewField label="Category" value={form.category} onChange={set("category")} confidence={extraction.company.category.confidence} /><ReviewField label="Website" value={form.website} onChange={set("website")} confidence={extraction.company.website.confidence} /><ReviewField label="Tagline" value={form.tagline} onChange={set("tagline")} confidence={extraction.company.tagline.confidence} /><ReviewField wide area label="Products & services" value={form.productsServices} onChange={set("productsServices")} confidence={extraction.company.productsServices.confidence} /></div></section>
